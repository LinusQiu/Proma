...
import type { AgentStreamEvent, AgentStreamCompletePayload, SDKMessage, PromaEvent, AgentSessionMeta, ProviderType } from '@proma/shared'
...
import { claimFinalToolSideEffects, isRunScopedRetryUpdate, isStaleAgentRunSignal, projectAgentLiveUpdates } from '@/lib/agent-canonical-stream'
  ...
  useEffect(() => {
    ...
    const handleStreamEvent = (streamEvent: AgentStreamEvent): void => {
        ...
        if (payload.kind === 'assistant_message_delta') {
          ...
          agentLiveTranscriptStore.apply(sessionId, payload)
        }
        ...
        if (payload.kind === 'sdk_message') {
          ...
          if (msgRecord.type === 'assistant' && msgRecord._partial !== true) {
    ...
    const cleanupComplete = window.electronAPI.onAgentStreamComplete(
      (data: AgentStreamCompletePayload) => {
        const activeStartedAt = store.get(agentStreamingStatesAtom).get(data.sessionId)?.startedAt
        if (isStaleAgentRunSignal(activeStartedAt, data.startedAt)) {
          console.warn(`[GlobalAgentListeners] 忽略旧 run 完成事件: session=${data.sessionId.slice(0, 8)}, active=${activeStartedAt}, incoming=${data.startedAt}`)
          return
        }
        // 无终态 assistant 的异常路径也不能让等待中的 partial 在完成后倒灌。
        streamEventBatcher.clear(data.sessionId)
        clearStartedTools(data.sessionId)
        agentLiveTranscriptStore.clear(data.sessionId)
        unstable_batchedUpdates(() => {
