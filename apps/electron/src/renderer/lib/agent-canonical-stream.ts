...
export function isStaleAgentRunSignal(
  ...
  return activeStartedAt !== undefined
    && (incomingStartedAt === undefined || activeStartedAt > incomingStartedAt)
}
...
function projectSystem(message: SDKSystemMessage): AgentLiveUpdate[] {
  ...
  if (message.subtype === 'thinking_tokens' && typeof message.estimated_tokens === 'number') {
    return [{
      type: 'thinking_tokens',
      estimatedTokens: message.estimated_tokens,
      estimatedTokensDelta: typeof message.estimated_tokens_delta === 'number' ? message.estimated_tokens_delta : 0,
    }]
  }
  return []
}
  ...
  switch (message.type) {
    ...
    case 'tool_use_summary': {
      ...
      return summary.summary
        ? [{ type: 'tool_use_summary', summary: summary.summary, precedingToolUseIds: summary.preceding_tool_use_ids ?? [] }]
        : []
    }
    default: return []
  }
